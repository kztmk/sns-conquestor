      ...
      {!column.disableSortBy && (
        ...
        >
          <ArrowUp2
            ...
            style={{
              color:
                column.isSorted && !column.isSortedDesc ? theme.palette.secondary.main : 'inherit',
            }}
          />
          <ArrowDown2
            size={16}
...
export const DragPreview = () => {
  ...
  const { isDragging, item, currentOffset } = useDragLayer((monitor: DragLayerMonitor<object>) => ({
...
export const DraggableRow = ({ index, moveRow, children }: DraggableRowProps) => {
  ...
  const [, drop] = useDrop({
    accept: DND_ITEM_TYPE,
    hover(item: any, monitor: DropTargetMonitor<object>) {
      if (!dropRef.current) {
        return;
      }
...
export const HidingSelect = ({
  ...
  const visible = allColumns.filter((c: ColumnInstance) => !hiddenColumns.includes(c.id)).length;
    ...
    <FormControl sx={{ width: 200 }}>
      ...
      >
        {allColumns.map((column: ColumnInstance) => {
          const ToggleChecked =
            column.id === '#' ? true : !(hiddenColumns!.indexOf(column.id) > -1);
          return (
            <MenuItem
              key={column.id}
              value={column.id}
